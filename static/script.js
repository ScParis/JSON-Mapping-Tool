...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function clearFields() {
        document.getElementById('sourceJsonTextarea').value = '';
        document.getElementById('outputJsonTextarea').value = '';
        document.getElementById('outputJsonContainer').value = '';
        document.getElementById('formContainer').innerHTML = '';
        document.getElementById('keysListbox').innerHTML = '';
    ...
    function setupDropZone(element, targetTextarea) {
        ...
        uploadButton.innerHTML = '<i class="fas fa-upload"></i> Arraste um arquivo JSON ou clique para selecionar';
    ...
    controlsDiv.innerHTML = `
        <button id="themeToggleBtn" class="btn-theme" data-tooltip="Alternar tema claro/escuro">
            <i id="themeIcon" class="fas fa-moon"></i>
        </button>
        <button id="loadExamplesBtn" class="btn-example" data-tooltip="Carregar JSONs de exemplo">
            <i class="fas fa-file-import"></i> Carregar Exemplos
        </button>
        <button id="clearFieldsBtn" class="btn-clear" data-tooltip="Limpar todos os campos">
    ...
    console.log('[init] mapButton encontrado:', !!mapButton);
    ...
    mapButton.addEventListener('click', async function () {
        console.log('[map] Botão clicado - Iniciando mapeamento');
        ...
        console.log('[map] Iniciando geração do formulário de mapeamento');
        console.log('[map] keysList disponível:', keysList.length, 'itens');
        console.log('[map] outputJson:', JSON.stringify(outputJson, null, 2));
        ...
        console.log('[map] Após inserir título, filhos no formContainer:', formContainer.children.length);
        const beforeCount = 0;
        try {
            console.log('[map] Chamando generateIndentedInputFields com outputJson e keysList');
            const formFields = generateIndentedInputFields(outputJson, keysList);
            console.log('[map] generateIndentedInputFields retornou:', formFields);
            if (formFields) {
                formContainer.appendChild(formFields);
                console.log('[map] formFields adicionado ao container');
            } else {
                console.warn('[map] generateIndentedInputFields retornou null/undefined');
            }
        } catch (err) {
