...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Define a função para validar o JSON de origem ao colar ou editar
    function validateJSON() {
        const sourceJsonText = sourceJsonTextarea.value;
        try {
            JSON.parse(sourceJsonText); // Tenta fazer o parse do JSON
            errorMessage.textContent = ''; // JSON válido, remove a mensagem de erro
        } catch (error) {
            errorMessage.textContent = 'JSON de origem inválido.'; // JSON inválido, exibe a mensagem de erro
        }
    }

    // Define o evento para o campo de entrada JSON quando colar ou editar
    sourceJsonTextarea.addEventListener('paste', validateJSON);
    sourceJsonTextarea.addEventListener('input', validateJSON);
});
